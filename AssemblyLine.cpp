...
void RunM3(std::ofstream& os)
{
	...
	{
		std::getline(file, record);
		Tasks.push_back(new Task(record));//calls constructor Task
	}

	file.close();

	for (size_t i = 0; i < Tasks.size(); i++)
		Tasks[i]->display(os, true); //itemclass display function
	...
	while (!file2.eof())
	{
		...
		CustomerOrder newOrder(record); 
		Orders.push_back(std::move(newOrder));//customerOrder class vector
	}
	...
	LineManager lm(filename, Tasks, Orders); //Linemanager constructor	LineManager(std::string&, std::vector<Task*>&, std::vector<CustomerOrder>&);
